...
├── extraction_parameters.xlsx # hand-off spec for a financial-statement extraction team
├── README.md                  # quickstart + method summary
└── PROJECT_CONTEXT.md         # THIS FILE — read §3a before assuming any input works
```
...
## 3a. THE MOCK DATABASE — what it is, where it lives, and every valid input

> **Read this before touching the UI or CLI.** This is the single most common
> confusion point for anyone (human or agent) new to this project, so it gets its
> own section instead of being buried in §5.

**There is no live company database.** There is no external API call, no real
D&B account, and no file you can point at called "the database." The entire
39,000-line-equivalent "universe of Indian companies" is a **59-row synthetic
dataset generated entirely in Python code**, deterministically, at import time:

- **Source of truth:** [`mock_api/dnb_mock.py`](mock_api/dnb_mock.py) — function
  `_build_universe()` (called once at module import into the module-level
  `_UNIVERSE` dict). Every financial figure, industry code, listing status, and
  market multiple is generated by seeded `random.Random(...)` calls, so the same
  seed always produces the same 59 companies with the same numbers. See §5 for the
  exact generation formulas.
- **Point-in-time export for inspection:** [`sample_database.json`](sample_database.json)
  at the project root — a static JSON dump of the current 59-company universe
  (name, DUNS, CIN, listing status, NAICS/Hoovers/major-industry codes,
  activities text, city, full financials in INR Thousand, and market data where
  listed). **This file is NOT the source of truth** — it is a snapshot for humans
  to read without running Python. If the generator code or its seeds ever change,
  regenerate this file; don't hand-edit it.
- **Do NOT** search the internet, invent, or assume real companies (e.g. a real
  "Woodward Inc." or "Kirloskar Brothers Limited") are connected to this system
  in any way. The names were chosen to *sound* like plausible Indian MSMEs in
  their sector, but every number attached to them is fabricated by the generator
  for testing purposes only.

### Every valid input — the only 59 strings the search will resolve

Typing anything **not** in this list into the UI search box or as `run.py`'s
argument produces a graceful `status: no_match` result (§10.4) — not an error,
not a crash, just an honest "found nothing." The live UI's `/api/companies`
endpoint returns exactly this same list for autocomplete, so typing and picking
from the dropdown can never miss.

**Cluster A — Turbine / engine controls (NAICS 333611, 18 companies):**
...
**Cluster B — Auto components (NAICS 336390, 18 companies):**
...
Minda Auto Parts · Lumax Auto Components · Fiem Auto Systems · Subros Auto Parts ·
Jamna Auto Components · Pricol Auto Systems

**Cluster C — Pumps & valves (NAICS 333914 / 332911, 18 companies):**
...
**The 5 "wrong" entities (5 companies)** — exist ONLY to demonstrate the
mismatch-filter rejection logic (§8) when they turn up as *candidate peers* for
one of the 54 companies above. Do not use these as the **target** of a search —
they have no same-category peers of their own, so a search on them will not
produce a meaningful valuation, only (at best) a thin/empty peer set:
Metro Wholesale Distributors · Prime Industrial Traders · Sterling Steel Billets ·
UrbanMart Retail Stores · Insight Engineering Consulting

**Total: 59.** Any 60th name does not exist in this build.

### For an agent extending this project

- To add a new testable company, add an entry to the appropriate cluster list (or
  a new cluster) inside `_build_universe()` in `mock_api/dnb_mock.py`, following
  the existing generation pattern (§5.1–5.2), then re-run
  `python -c "..."` (see the export snippet used for `sample_database.json`) to
  refresh the snapshot if one is needed.
- To go live against the real D&B API, see §12 — the universe generator is
  replaced by paging the real `company_search`, and this whole section becomes
  moot (any real company name becomes a valid input).
